    ...
    runButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (imports.length > 0) {
                ...
                const builtInModules = ['re', 'math', 'os', 'sys', 'random', 'datetime', 'json', 'collections', 'itertools', 'functools', 'textwrap', 'abc'];

                imports.forEach(pkg => {
                    if (builtInModules.includes(pkg)) {
                        // This is a built-in module, do nothing.
                        return;
                    }
                    const mappedPkg = packageMap[pkg] || pkg;
                    if (knownPyodidePackages.includes(mappedPkg)) {
                        pyodidePackagesToLoad.add(mappedPkg);
                    } else {
                        pipPackagesToLoad.add(mappedPkg);
                    }
                });
